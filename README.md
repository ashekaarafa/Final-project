...
The daily water level at Hofkirchen, Danube River are available from 2021.12.31  through 2022.01.01 in the form of `.csv` files 
([`my_data` folder])(https://github.com/sahasandip/Final-project/tree/main/my_data). The data is downloded from the link (https://www.bafg.de/DE/Home/homepage_node.html)
...
1. _my_config.py_ : Import all necessary modules and packages
  2. package folder "my_package" contains four files

      **init.py** : contains dataedit, modelling and classical_arima.
  ...
  3. _main.py:_ The above  files are plugged in "main.py" to calculate the future water levels.
  4. _main_2.py:_ Create several plots (autocorrelation, partial autocorrelation plot). After visualizing the graph parameters of Arima model are selected.
...
_**runing the main.py file will print this:**_
